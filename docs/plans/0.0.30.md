...
- `/world create`：创建世界（生成草稿 + 私密话题多轮补全；在话题中 `/world done` 发布后创建子空间）
- `/world edit <worldId>`：创建世界编辑话题（仅创作者；私密可见）
- `/world done`：结束构建/编辑话题（草稿会在此时发布；已发布世界则仅结束话题）
- `/world list`：列出所有可探索世界（全局）
- `/world info <worldId>`：查看世界卡与入口信息（全局可读）
- `/world rules <worldId>`：查看世界规则与底层逻辑（全局可读）
- `/world join`：加入世界（仅在世界子空间的 `world-join` 频道执行；必须在 `homeGuild` 执行以赋 role）
