...
- agent.md frontmatter 使用 Zod 校验。

待完成：

- 补充断线重连与消息二进制数据的单元测试。
- 补充发送消息构造与校验的单元测试。
- 处理手动 connect 与自动重连定时器的竞态。
- 恢复文件变更的防抖，避免重复 reload。
- 评估 GroupStore 拆分与 frontmatter 解析库替换。
- 评估 CONFIG_PATH 环境变量替代硬编码路径。
...
- [ ] **M4**: 测试补齐 (Reconnect / Binary Message / Sender)
