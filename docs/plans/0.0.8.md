...
- [x] **Store 重构**: 适配 `/data/groups/{gid}` + `/data/groups/sessions/{botId}/{groupId}/{userId}/{sessionId}` 结构。
- [x] **Session Manager**:
  - 实现 `createSession(groupId, userId, key=0)`:
    - 如果 `key >= maxSessions`，则拒绝。
    - 目录名: `sessions/{botId}/{groupId}/{userId}/{sessionId}`（`sessionId = {userId}-{key}`）。
  - 实现 `meta.json` 管理。
- [x] **Opencode Launcher**:
  - 确保 `cwd = sessions/{botId}/{groupId}/{userId}/{sessionId}/workspace`。
  - 确保只读挂载/访问上层资源。
