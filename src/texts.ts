...
export function buildDiscordWorldBuildKickoff(input: {
  ...
  return pick(
    ...
    [
      "你现在在世界构建/编辑模式。",
      "请先读取「世界书：原始资料」，然后用技能 world-design-card 规范化并更新：",
      "- 「世界书：世界卡」",
      "- 「世界书：规则」（底层规则，如初始金额/装备等）",
      "",
      "要求：",
      ...
      "5) 面向创作者的回复里禁止出现任何工作区路径/文件名；统一使用“世界书/角色卡”等产品术语。",
      "",
    ...
    [
      ...
      "First read “Worldbook: Source”, then use skill world-design-card to normalize and update:",
      "- “Worldbook: World Card”",
      "- “Worldbook: Rules” (hard rules, e.g. starting money/equipment)",
      "",
      "Requirements:",
      ...
      "5) In creator-facing replies, never expose workspace paths or filenames; use product terms such as Worldbook / Character Card.",
      "",
...
export function buildDiscordWorldBuildAutopilot(input: {
  ...
  return pick(
    input.language,
    [
      "自动推进（Autopilot）：请基于当前「世界书：原始资料」，自动整理并完善：",
      "- 「世界书：世界卡」",
      "- 「世界书：规则」",
      "",
    ...
    [
      "Autopilot: based on the current “Worldbook: Source”, normalize and refine:",
      "- “Worldbook: World Card”",
      "- “Worldbook: Rules”",
      "",
...
export function buildDiscordCharacterBuildAutopilot(input: {
  ...
  return pick(
    input.language,
    [
      "自动推进（Autopilot）：请基于当前「角色图书馆：原始资料」，自动整理并完善：",
      "- 「角色卡」",
      "",
      "能确定的先写；不确定的请标注并在末尾列出 3-5 个需要我确认的问题。",
      "不要 roleplay，不要编造未给出的设定；禁止使用交互式 question 工具。",
    ...
    [
      "Autopilot: based on the current “Character Library: Source”, normalize and refine:",
      "- “Character Card”",
      "",
      "Write what you can first; clearly mark uncertainties, then list 3–5 questions for me to confirm.",
      "Do not roleplay; do not invent details not provided; do not use interactive question tools (e.g. question).",
...
export function buildDiscordCharacterBuildKickoff(input: {
  ...
  return pick(
    ...
    [
      "你现在在角色卡构建模式。",
      "请先读取「角色图书馆：原始资料」，然后完善并更新：",
      "- 「角色卡」（可写）",
      "",
      "请使用技能 character-card 完善并更新「角色卡」。",
      "",
      "要求：",
      "1) 必须通过工具写入/编辑文件，不能只在聊天里输出。",
    ...
    [
      ...
      "First read “Character Library: Source”, then refine and update:",
      "- “Character Card” (writable)",
      "",
      "Use skill character-card to refine and update the “Character Card”.",
      "",
      "Requirements:",
      "1) You must write/edit files via tools; do not output only in chat.",
...
export function buildDiscordWorldCharacterBuildKickoff(input: {
  ...
  return pick(
    ...
    [
      ...
      "",
      "Read:",
      "- “Worldbook: World Card” (read-only)",
      "- “Worldbook: Rules” (read-only)",
      "",
      "Update:",
      "- “Character Card” (writable)",
      "",
      "Requirements:",
      "1) You must write/edit files via tools; do not output only in chat.",
      "2) Do not modify “Worldbook: World Card” or “Worldbook: Rules” (read-only).",
...
export function buildWorldAgentPrompt(input: {
  ...
  return pick(
    ...
    [
      ...
      "4) 内部可使用工作区路径读取资料，但对用户的回复里禁止出现任何路径/文件名；统一使用“世界书/角色卡”等产品术语。",
      "",
    ].join("\n"),
    [
      "---",
      ...
      "4) You may use workspace paths internally, but never expose any path or filename in user-facing replies; use product terms like Worldbook / Character Card.",
      "",
    ].join("\n"),
  );
}
...
export function buildWorldBuildAgentPrompt(input: {
  ...
  return pick(
    ...
    [
      ...
      "补充：你可以在内部使用这些路径，但对创作者的回复里禁止出现任何路径/文件名；统一使用“世界书/角色卡”等产品术语。",
    ...
    [
      ...
      "Supplement: you may use these paths internally, but never expose any path or filename in creator-facing replies; use product terms like Worldbook / Character Card.",
...
export function buildCharacterBuildAgentPrompt(input: {
  ...
  return pick(
    ...
    [
      ...
      "补充：你可以在内部使用这些路径，但对创作者的回复里禁止出现任何路径/文件名；统一使用“角色图书馆/角色卡”等产品术语。",
    ...
    [
      ...
      "Supplement: you may use these paths internally, but never expose any path or filename in creator-facing replies; use product terms like Character Library / Character Card.",
      "3) Each reply must include: change summary + (if info is missing) 3–5 follow-up questions; if sufficient, explicitly say “OK”.",
      "4) Do not use interactive question tools (e.g. question); list follow-up questions as plain text.",
      "5) You are not here to write a novel; do not roleplay.",
      "",
...
export function buildWorldCharacterBuildAgentPrompt(input: {
  ...
  return pick(
    ...
    [
      ...
      "补充：你可以在内部读取这些路径，但对创作者的回复里禁止出现任何路径/文件名；统一使用“世界书/角色卡”等产品术语。",
    ...
    [
      ...
      "Supplement: you may read these paths internally, but never expose any path or filename in creator-facing replies; use product terms like Worldbook / Character Card.",
