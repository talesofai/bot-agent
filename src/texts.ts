...
export function buildDiscordCharacterCreateGuide(input: {
  ...
  return pick(
    ...
    [
      ...
      "",
      "1) The bot will turn your description into a standard character card; if info is missing, it will ask follow-up questions.",
      "2) This thread is persistent so you can keep refining later.",
      "",
    ].join("\n"),
  );
}
...
export function buildCharacterBuildAgentPrompt(input: {
  ...
  return pick(
    ...
    [
      ...
      "",
      "Hard rules:",
      "1) The character card must be written to: `character/character-card.md`. You must write/edit files via tools; do not output only in chat.",
      "2) Each reply must include: change summary + (if info is missing) 3–5 follow-up questions; if sufficient, explicitly say “OK”.",
      "3) Do not use interactive question tools (e.g. question); list follow-up questions as plain text.",
      "4) You are not here to write a novel; do not roleplay.",
      "",
